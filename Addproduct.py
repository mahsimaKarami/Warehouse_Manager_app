...
from PySide6.QtGui import QIcon,QTextDocument,QPageLayout
from PySide6.QtCore import Qt,QMarginsF
from Addproduct_ui import Ui_Addproduct
import sqlite3
...
class Addproduct(QWidget,Ui_Addproduct):
    def __init__(self,mainWindow):
        super().__init__()
        self.setupUi(self)
        self.setWindowTitle("Add product to the receipt of receive")
        ...
        self.tableWidget.setHorizontalHeaderLabels(["Barcode","Name","Price","Amount","Total price","Delete"])
    ...
    def add(self):
        stock = self.lineEdit.text().strip()
        data = self.comboBox.currentText()
        if len(stock) == 0:
            QMessageBox.critical(self,"Error!!","you have to enter stock of how many you receive. Type the number of stock in the box below the page")
        elif not(stock.isdigit()):
            QMessageBox.critical(self,"Error!!","you have to enter a number for stock. Type the number of stock in the box below the page")
            self.lineEdit.setText("")
        elif self.duplicateInTable(data):
            QMessageBox.critical(self,"Error!!","This item exists in table. If you want to change stock,you have to double click on stock of it in table")
            self.lineEdit.setText("")
        else:
            selected_id = self.comboBox.currentData()
            conn = sqlite3.connect("warehouse.db")
            cursor = conn.cursor()
            cursor.execute("SELECT barcode,name,price FROM products WHERE id = ?",(int(selected_id),))
            ...
            self.tableWidget.setItem(row, 3, stock_item)
            self.tableWidget.setItem(row,4,total_item)
            btn = QPushButton()
    ...
    def calculate_total(self):
        ...
        for row in range(self.tableWidget.rowCount()):
            total  += float(self.tableWidget.item(row,4).text().strip())
    ...
    def itemChange(self,item):
        if item.column() == 3:
            ...
            if not stock.isdigit():
                ...
                self.tableWidget.setItem(item.row(),4,newTotal_item)
                self.tableWidget.blockSignals(False)
                self.calculate_total()
            else:
    ...
    def addtodb(self):
        ...
        if self.tableWidget.rowCount() == 0:
            QMessageBox.critical(self,"Error!!","If the table is empty, you can't press submit. You have to enter at least one product in table. Use combo box for choosing a product and type the number of stock in the box below the page. \n If you don't have nothing to add, just turn back to dashboard by home button top of the page")
        else:
            cursor.execute("INSERT INTO transactions (receiver,sender,type,date,description,totalprice) VALUES (?,?,?,?,?,?)",(self.rec,self.send,"receive",self.date,self.desc,self.total))
            conn.commit()
            new_id = cursor.lastrowid
            for row in range(self.tableWidget.rowCount()):
                barcode = self.tableWidget.item(row,0).text().strip()
                total = float(self.tableWidget.item(row,4).text().strip())
                stock = int(self.tableWidget.item(row,3).text().strip())
                cursor.execute("SELECT id,stock FROM products WHERE barcode = ?",(barcode,))
                data = cursor.fetchall()
                totalStock = stock + data[0][1]
